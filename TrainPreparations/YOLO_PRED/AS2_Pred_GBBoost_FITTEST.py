...
if __name__ == "__main__":
    ...
    # Grid search values
    GB_thresholds = [0.2, 0.3, 0.4]
    GB_scoreweights = [0.2, 0.3, 0.4]

    Detector_model_path = "C:/PHD/SideProjectsData/IcasspExperiments/YOLOExperiments/TrainingData/Models/ACA_26_M_ICASSP/weights/best.pt"
    GarbageFilter_model_path = "C:/PHD/SideProjectsData/IcasspExperiments/YOLOExperiments/TrainingData/GB_Boost_Model/XGBRich.joblib"

    audio_folder = "C:/PHD/SideProjectsData/IcasspExperiments/YOLOExperiments/ValidateData/ACA/"

    # Parent output folder
    parent_output = "C:/PHD/SideProjectsData/IcasspExperiments/YOLOExperiments/ValidateData/FitGBScoreThresh_Boost/"

    os.makedirs(parent_output, exist_ok=True)

    total_runs = len(GB_thresholds) * len(GB_scoreweights)
    run_id = 1

    for t in GB_thresholds:
        for w in GB_scoreweights:

            print("\n======================================")
            print(f" RUN {run_id}/{total_runs}")
            print(f" GB_threshold = {t}")
            print(f" GB_scoreweight = {w}")
            print("======================================\n")

            # Create unique output folder
            run_folder_name = f"GBt_{t:.2f}_GBw_{w:.2f}"
            output_path = os.path.join(parent_output, run_folder_name)
