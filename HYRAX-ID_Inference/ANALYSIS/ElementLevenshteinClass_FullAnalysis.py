...
def parse_file(filepath, is_pred=False):
    ...
    Bouts are returned as (start, end, raw_label) tuples. raw_label keeps
    its ORIGINAL casing (not lower-cased) so that speaker-class tokens like
    "P0" or "Kashtan" embedded in predicted bout labels
    (e.g. "BOUT1_P0_0.145") can be recovered correctly later.
    """
    ...
    with open(filepath) as f:
        for line in f:
            ...
            raw_label = parts[2]
            label_check = raw_label.lower()

            # Bout lines (GT: "bout_0" / Pred: "BOUT1_P0_0.145")
            if "bout" in label_check:
                bouts.append((start, end, raw_label))
                continue
    ...
    for bout_start, bout_end, _ in bouts:
...
def extract_pred_bout_class(raw_label):
    """
    Extract the predicted speaker/class token from a predicted bout label
    of the form "BOUT{num}_{class}_{classconf}", e.g. "BOUT1_P0_0.145" -> "P0".

    Returns None if the label doesn't match the expected pattern (e.g. the
    old-style GT bout label "bout_0", which carries no class info).
    """
...
def get_true_class_from_filename(filepath):
    """
    GT files are named like 'P0_Take159_P0_070409.txt' or
    'Q7_Take110_Q7_040903.txt' -> the class is the token before the first
    underscore.
    """
...
# =========================
# BOUT EVAL (element-sequence Levenshtein, unchanged from original)
# =========================
...
# =========================
# NEW: BOUT-LEVEL SPEAKER/CLASS PERFORMANCE
# =========================
def build_confusion_matrix(class_pairs, speaker_list):
    """
    class_pairs: list of (true_class, pred_class, source_file)
    pred_class may be None if it couldn't be parsed from the bout label.

    Any true/pred class not present in speaker_list is bucketed separately
    (true classes -> their own literal label, so mismatched filenames are
    visible; unparsable predictions -> 'UNPARSED').
    """
    extra_labels = set()
    for true_c, pred_c, _ in class_pairs:
        if true_c not in speaker_list:
            extra_labels.add(true_c)
        if pred_c is None:
            extra_labels.add("UNPARSED")
        elif pred_c not in speaker_list:
            extra_labels.add(pred_c)

    labels = list(speaker_list) + sorted(extra_labels)
    idx = {c: i for i, c in enumerate(labels)}
    n = len(labels)
    matrix = np.zeros((n, n), dtype=int)

    for true_c, pred_c, _ in class_pairs:
        p = pred_c if pred_c is not None else "UNPARSED"
        matrix[idx[true_c]][idx[p]] += 1

    return matrix, labels


def compute_class_performance(matrix, labels, speaker_list):
    """
    Precision / Recall / F1 per class (restricted to the official
    SPEAKER_LIST classes), macro-averaged over classes that actually
    appear in the test set, plus overall accuracy.
    """
    idx = {c: i for i, c in enumerate(labels)}
    total = matrix.sum()
    ...
    # Only average over classes that actually occur in the test set
    valid_classes = [c for c in speaker_list if per_class[c]["support"] > 0]

    if valid_classes:
        macro_precision = np.mean([per_class[c]["precision"] for c in valid_classes])
        macro_recall = np.mean([per_class[c]["recall"] for c in valid_classes])
        macro_f1 = np.mean([per_class[c]["f1"] for c in valid_classes])
    else:
...
def print_confusion_matrix(matrix, labels):
    ...
    header = "TRUE\\PRED".rjust(12) + "".join(l.rjust(col_w) for l in labels)
    print(header)
    for i, row_label in enumerate(labels):
        row_str = row_label.rjust(12) + "".join(str(matrix[i, j]).rjust(col_w) for j in range(len(labels)))
        print(row_str)


def print_class_performance(perf):
    print(f"\nOverall bout-classification accuracy: {perf['accuracy']:.4f}")
    print(f"Macro Precision: {perf['macro_precision']:.4f}")
    print(f"Macro Recall:    {perf['macro_recall']:.4f}")
    print(f"Macro F1:        {perf['macro_f1']:.4f}\n")

    print(f"{'Class':<10}{'Support':>8}{'TP':>6}{'FP':>6}{'FN':>6}{'Prec':>9}{'Rec':>9}{'F1':>9}")
    for c, m in perf["per_class"].items():
        print(f"{c:<10}{m['support']:>8}{m['TP']:>6}{m['FP']:>6}{m['FN']:>6}"
...
def main(gt_folder, pred_folder):
    ...
    for gt_file in glob.glob(os.path.join(gt_folder, "*.txt")):
        ...
        pred, pred_bouts = parse_file(pred_file, True)

        all_gt.extend(gt)
        all_pred.extend(pred)
        ##Match here?
        # --- NEW: bout-level class comparison ---
        true_class = get_true_class_from_filename(gt_file)
        if true_class not in SPEAKER_LIST:
            print(f"WARNING: '{true_class}' (from {base}) not in SPEAKER_LIST")

        for (b_start, b_end, raw_label) in pred_bouts:

            pred_class = extract_pred_bout_class(raw_label)

            if pred_class is None:
                print(f"WARNING: could not parse class from bout label "
                      f"'{raw_label}' in {base}")

            # Determine the bout_id belonging to this predicted bout
            bout_id = None

            for elem in pred:
                if elem.bout_id != -1 and b_start <= elem.start < b_end:
                    bout_id = elem.bout_id
                    break
    ...
    print("\n=== BOUT RESULTS (element-sequence Levenshtein) ===")
    ...
    for record in bout_records:

        if record["bout_id"] in matched_pred_bouts:
    ...
    print("\n=== BOUT CLASS (SPEAKER) PERFORMANCE ===")
    matrix, labels = build_confusion_matrix(class_pairs, SPEAKER_LIST)
    perf = compute_class_performance(matrix, labels, SPEAKER_LIST)

    print("\n--- Confusion Matrix (rows=true, cols=pred) ---")
    print_confusion_matrix(matrix, labels)

    print_class_performance(perf)
