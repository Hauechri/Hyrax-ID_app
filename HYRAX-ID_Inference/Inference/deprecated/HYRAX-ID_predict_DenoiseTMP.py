...
def denoise_audio_folder(model, dataOpts, audio_folder, denoised_output_dir,
                          sequence_len_seconds, num_workers, device):
    """
    Runs the denoiser over every wav file in `audio_folder` and writes the
    denoised result to `denoised_output_dir` under the same stem + ".wav".

    Replaces the old "two pre-populated folders + match_audio_pairs"
    approach: the denoised folder is now produced on the fly instead of
    being a required input, so there's no "missing counterpart" case to
    warn about -- every original file that denoises successfully gets a
    pair.

    Returns a list of (stem, original_path, denoised_path) triples, in the
    same shape match_audio_pairs used to return, so the rest of the
    pipeline (detect_elements_for_file, etc.) needs no other changes.
    """
    ...
    with torch.no_grad():
        for stem, original_path in sorted(originals.items()):
            print(f"[DENOISE] Processing: {original_path}")
            ...
            total_audio_np = total_audio.cpu().numpy()
            total_audio_np = total_audio_np * np.iinfo(np.int16).max
            total_audio_np = total_audio_np.astype(np.int16)

            denoised_path = os.path.join(denoised_output_dir, f"{stem}.wav")
            wav.write(denoised_path, sr, total_audio_np)

            pairs.append((stem, original_path, denoised_path))
...
def run_pipeline(
    ...
    # Denoise every file in audio_folder on the fly (replaces the old
    # pre-populated denoised folder + match_audio_pairs step).
    pairs = denoise_audio_folder(
        denoiser_model,
        dataOpts,
        audio_folder,
        denoised_output_dir,
    ...
    print(f"[INFO] Denoised {len(pairs)} file(s).")
...
if __name__ == "__main__":
    ...
    # Denoiser settings -- mirrors the predict.py CLI:
    #   --model_path MODEL_PATH --sequence_len 1 --num_workers 0
    #   (no --checkpoint_path, no --jit_load, no --min_max_norm, no log_dir)
    Denoiser_model_path = "C:/PHD/SideProjects/ANIMAL-CLEAN/01_ACS.pk"
    ...
    denoised_output_dir = os.path.join(output_path, "denoised")
    ...
    run_pipeline(
        ...
        denoised_output_dir=denoised_output_dir,
